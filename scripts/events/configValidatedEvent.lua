...
function ConfigValidatedEvent.new(hub,slotIndex,bValid)
...
function ConfigValidatedEvent:run(connection)
    if self.hub ~= nil then
        self.hub:validatedSlotSettings(self.slotIndex,self.bValid)
    end
end
...
--@param slotIndex is the index of slot of the hub that has settings changed.
--@param bValid indicates if the new settings received(path was able to be created) were valid.
function ConfigValidatedEvent.sendEvent(hub,slotIndex,bValid)
    if hub ~= nil and slotIndex ~= nil and g_server ~= nil then
        g_server:broadcastEvent(ConfigValidatedEvent.new(hub,slotIndex,bValid), nil, nil, hub)
        hub:validatedSlotSettings(slotIndex,bValid)
    end
end